...
const cloudinary = require('cloudinary').v2; // Importar el módulo de Cloudinary
...
const register = async (name, email, password, userName, lastName, birthDate, profileImage, role) => {
  ...
  if (profileImage) {
    try {
     ...
     const newUser = await User.create( { name, email, userName, lastName, birthDate, profileImage: cloudinaryUrl, password: passwordHash, role } )
     const responseUser = {
       id: newUser.id,
       name: newUser.name,
